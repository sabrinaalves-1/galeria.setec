...
btnFromUrl.addEventListener('click',async()=>{
  ...
  try{await addImageFromUrl(url); alert('Imagem adicionada!');}
  catch(e){alert('Erro: '+e.message);}
});
btnClearAll.addEventListener('click',()=>{if(confirm('Remover todas as imagens?')){items=[];save();render();}});

fileInput.addEventListener('change',async ev=>{const files=Array.from(ev.target.files||[]);for(const f of files) await addFile(f); fileInput.value='';});

// Drag & Drop
['dragenter','dragover'].forEach(ev=>dropzone.addEventListener(ev,e=>{e.preventDefault();dropzone.classList.add('drag')}));
['dragleave','drop'].forEach(ev=>dropzone.addEventListener(ev,e=>{e.preventDefault();dropzone.classList.remove('drag')}));
dropzone.addEventListener('drop',async e=>{const dt=e.dataTransfer;if(!dt)return; const files=Array.from(dt.files||[]); for(const f of files) await addFile(f);});
...
function openModal(item){modalImg.src=item.dataUrl;modalDesc.textContent=item.desc||('Adicionada em '+new Date(item.createdAt).toLocaleString());currentScale=1; modalImg.style.transform='scale(1)'; modal.classList.remove('hidden'); modal.setAttribute('aria-hidden','false');}
...
async function addFile(file){
  ...
  let dia=parseInt(prompt('Adicionar imagem ao Dia 1,2 ou 3?','1'));
  if(isNaN(dia)||dia<1||dia>3) dia=1;
  items.unshift({id:randomId(),dataUrl:watermarked,desc:file.name,dia,createdAt:Date.now()});
  save(); render();
}
async function addImageFromUrl(url){
  ...
  let dia=parseInt(prompt('Adicionar imagem ao Dia 1,2 ou 3?','1'));
  if(isNaN(dia)||dia<1||dia>3) dia=1;
  items.unshift({id:randomId(),dataUrl:watermarked,desc:url,dia,createdAt:Date.now()});
...
function applyWatermark(dataUrl){return new Promise(res=>{const img=new Image(); img.crossOrigin='anonymous'; img.onload=()=>{const canvas=document.createElement('canvas');canvas.width=img.width;canvas.height=img.height;const ctx=canvas.getContext('2d'); ctx.drawImage(img,0,0); ctx.fillStyle='rgba(0,0,0,0.2)'; ctx.fillRect(0,img.height-50,img.width,40); ctx.fillStyle='rgba(255,255,255,0.8)'; ctx.font=Math.round(Math.max(12,img.width*0.02))+'px sans-serif'; ctx.textAlign='left'; ctx.fillText('Galeria • Barbosa',10,img.height-15); res(canvas.toDataURL('image/jpeg',0.9));}; img.onerror=()=>res(dataUrl); img.src=dataUrl;});}
