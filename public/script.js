...
btnClearAll.addEventListener('click', ()=>{
  if(confirm('Remover todas as imagens?')){
    items=[];
    save();
    render();
  }
...
zoomIn?.addEventListener('click', ()=>{
  if(currentScale < maxScale){ currentScale += 0.2; updateZoom(); } 
});
zoomOut?.addEventListener('click', ()=>{
  if(currentScale > minScale){ currentScale -= 0.2; updateZoom(); } 
});
...
// --- Upload para Node.js (atualizado) ---
async function uploadFileToServer(file){
  ...
  try{
    ...
    const item = {
      id: randomId(),
      dataUrl: data.url,       // URL retornada pelo servidor
      desc: file.name,
